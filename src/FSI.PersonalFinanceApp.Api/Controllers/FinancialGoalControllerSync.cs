...
public class FinancialGoalControllerSync : BaseSyncController
{
    private readonly IFinancialGoalAppService _service;

    public FinancialGoalControllerSync(
        IFinancialGoalAppService service,
    ...
    [HttpGet]
    [ProducesResponseType(typeof(IEnumerable<FinancialGoalDto>), StatusCodes.Status200OK)]
    ...
    {
        Track("GET - GetAll - FinancialGoal - Sync", "Request");
        var result = _service.GetAllSync();
        Track("GET - GetAll - FinancialGoal - Sync", "Response");
    ...
    {
        Track("GET - GetById - FinancialGoal - Sync", "Request");
        var result = _service.GetByIdSync(id);
        Track("GET - GetById - FinancialGoal - Sync", "Response");
    ...
    {
        // [ApiController] já retorna 400 ProblemDetails para ModelState inválido.
        Track("POST - Create - FinancialGoal - Sync", "Request");
        _service.AddSync(dto);
        Track("POST - Create - FinancialGoal - Sync", "Response");
    ...
    public IActionResult Update(long id, [FromBody] FinancialGoalDto dto)
    {
        ...
        Track("PUT - Update - FinancialGoal - Sync", "Request");
        ...
        Track("PUT - Update - FinancialGoal - Sync", "Response");
        return NoContent();
    }
    ...
    {
        Track("DELETE - Delete - FinancialGoal - Sync", "Request");
        var existing = _service.GetByIdSync(id);
        if (existing is null) return NotFound(); // ou: throw new NotFoundException(...)
        _service.DeleteSync(existing);
        Track("DELETE - Delete - FinancialGoal - Sync", "Response");
        return NoContent();
    }
    ...
    public IActionResult GetAllFiltered([FromQuery] string filterBy, [FromQuery] string value)
    {
        ...
        Track("GET - GetAllFiltered - FinancialGoal - Sync", "Request");
        var result = _service.GetAllFilteredSync(filterBy, value);
        Track("GET - GetAllFiltered - FinancialGoal - Sync", "Response");
        return OkOrNoContent(result);
    }
    ...
    public IActionResult GetAllOrdered([FromQuery] string orderBy, [FromQuery] string direction = "asc")
    {
        ...
        Track("GET - GetAllOrdered - FinancialGoal - Sync", "Request");
        var result = _service.GetAllOrderedSync(orderBy, direction);
        Track("GET - GetAllOrdered - FinancialGoal - Sync", "Response");
        return OkOrNoContent(result);
    }
