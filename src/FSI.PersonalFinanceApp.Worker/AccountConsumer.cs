...
namespace FSI.PersonalFinanceApp.Worker
{
    public class AccountConsumer : BackgroundService
    {
        ...
        public AccountConsumer(IServiceScopeFactory scopeFactory, IConfiguration config)
        {
        ...
        {
            string queueName = "account-queue";
            ...
            consumer.Received += async (sender, ea) =>
            {
                try
                {
                    ...
                    var envelope = JsonSerializer.Deserialize<AccountMessage>(
                        message,
                    ...
                    var service = scope.ServiceProvider.GetRequiredService<IAccountAppService>();
                    ...
                    IEnumerable<AccountDto> listAccount = null;

                    AccountDto expenseCategory = null;
                    ...
                    switch (envelope.Action.ToLowerInvariant())
                    {
                        case "create":
                            ...
                            break;
                        case "getall":
                            listAccount = await service.GetAllAsync();
                            break;
                        case "getbyid":
                    ...
                    await ProcessedMessageAsync(messagingService, envelope, queueName, createdId, isDone, listAccount, expenseCategory);
        ...
        private async Task ProcessedMessageAsync(IMessagingAppService messagingService, AccountMessage envelope, string queueName, long? createdId, bool isDone,
            IEnumerable<AccountDto> listAccount, AccountDto expenseCategory)
        {
            ...
            {
                await ProcessedMessageGetAllAsync(messagingService, envelope, queueName, listAccount);
            }
        ...
        private async Task ProcessedMessageCreateAsync(IMessagingAppService messagingService, AccountMessage envelope,
        ...
        private async Task ProcessedMessageGetAllAsync(IMessagingAppService messagingService, AccountMessage envelope,
            string queueName, IEnumerable<AccountDto> listAccount)
        {
            if (listAccount != null)
            {
                var updatedResponse = JsonSerializer.Serialize(listAccount);
            ...
            else
            {
                await messagingService.UpdateAsync(new MessagingDto
                {
                    ...
                    QueueName = queueName,
                    MessageRequest = string.Empty,
                    MessageResponse = JsonSerializer.Serialize(listAccount),
                    IsProcessed = false,
                    ErrorMessage = "Failed to insert expense category into database.",
                    UpdatedAt = DateTime.Now,
                    IsActive = false
        ...
        private async Task ProcessedMessageGetByIdAsync(IMessagingAppService messagingService, AccountMessage envelope,
            string queueName, AccountDto expenseCategory)
        {
        ...
        private async Task ProcessedMessageUpdateAsync(IMessagingAppService messagingService, AccountMessage envelope,
        ...
        private async Task ProcessedMessageDeleteAsync(IMessagingAppService messagingService, AccountMessage envelope,
            string queueName, bool isDone)
        {
            if (isDone)
            {
