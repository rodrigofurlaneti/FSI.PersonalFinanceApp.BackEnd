...
namespace FSI.PersonalFinanceApp.Worker
{
    public class TransactionConsumer : BackgroundService
    {
        ...
        public TransactionConsumer(IServiceScopeFactory scopeFactory, IConfiguration config)
        {
        ...
        {
            string queueName = "transaction-queue";
            ...
            consumer.Received += async (sender, ea) =>
            {
                try
                {
                    ...
                    var envelope = JsonSerializer.Deserialize<TransactionMessage>(
                        message,
                    ...
                    var service = scope.ServiceProvider.GetRequiredService<ITransactionAppService>();
                    ...
                    IEnumerable<TransactionDto> listTransaction = null;

                    TransactionDto expenseCategory = null;
                    ...
                    switch (envelope.Action.ToLowerInvariant())
                    {
                        case "create":
                            ...
                            break;
                        case "getall":
                            listTransaction = await service.GetAllAsync();
                            break;
                        case "getbyid":
                    ...
                    await ProcessedMessageAsync(messagingService, envelope, queueName, createdId, isDone, listTransaction, expenseCategory);
        ...
        private async Task ProcessedMessageAsync(IMessagingAppService messagingService, TransactionMessage envelope, string queueName, long? createdId, bool isDone,
            IEnumerable<TransactionDto> listTransaction, TransactionDto expenseCategory)
        {
            ...
            {
                await ProcessedMessageGetAllAsync(messagingService, envelope, queueName, listTransaction);
            }
        ...
        private async Task ProcessedMessageCreateAsync(IMessagingAppService messagingService, TransactionMessage envelope,
        ...
        private async Task ProcessedMessageGetAllAsync(IMessagingAppService messagingService, TransactionMessage envelope,
            string queueName, IEnumerable<TransactionDto> listTransaction)
        {
            if (listTransaction != null)
            {
                var updatedResponse = JsonSerializer.Serialize(listTransaction);
            ...
            else
            {
                await messagingService.UpdateAsync(new MessagingDto
                {
                    ...
                    QueueName = queueName,
                    MessageRequest = string.Empty,
                    MessageResponse = JsonSerializer.Serialize(listTransaction),
                    IsProcessed = false,
                    ErrorMessage = "Failed to insert expense category into database.",
                    UpdatedAt = DateTime.Now,
                    IsActive = false
        ...
        private async Task ProcessedMessageGetByIdAsync(IMessagingAppService messagingService, TransactionMessage envelope,
            string queueName, TransactionDto expenseCategory)
        {
        ...
        private async Task ProcessedMessageUpdateAsync(IMessagingAppService messagingService, TransactionMessage envelope,
        ...
        private async Task ProcessedMessageDeleteAsync(IMessagingAppService messagingService, TransactionMessage envelope,
            string queueName, bool isDone)
        {
            if (isDone)
            {
