...
{
    public class IncomeConsumer : BackgroundService
    {
        private readonly IServiceScopeFactory _scopeFactory;
        private readonly IConfiguration _config;

        public IncomeConsumer(IServiceScopeFactory scopeFactory, IConfiguration config)
        {
        ...
        protected override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.Received += async (sender, ea) =>
            {
                try
                {
                    ...
                    var envelope = JsonSerializer.Deserialize<IncomeMessage>(
                        message,
                    ...
                    var service = scope.ServiceProvider.GetRequiredService<IIncomeAppService>();
                    ...
                    IEnumerable<IncomeDto> listIncome = null;

                    IncomeDto expenseCategory = null;
                    ...
                    switch (envelope.Action.ToLowerInvariant())
                    {
                        case "create":
                            ...
                            break;
                        case "getall":
                            listIncome = await service.GetAllAsync();
                            break;
                        case "getbyid":
                    ...
                    await ProcessedMessageAsync(messagingService, envelope, queueName, createdId, isDone, listIncome, expenseCategory);
        ...
        private async Task ProcessedMessageAsync(IMessagingAppService messagingService, IncomeMessage envelope, string queueName, long? createdId, bool isDone,
            IEnumerable<IncomeDto> listIncome, IncomeDto expenseCategory)
        {
            ...
            {
                await ProcessedMessageGetAllAsync(messagingService, envelope, queueName, listIncome);
            }
        ...
        private async Task ProcessedMessageCreateAsync(IMessagingAppService messagingService, IncomeMessage envelope,
        ...
        private async Task ProcessedMessageGetAllAsync(IMessagingAppService messagingService, IncomeMessage envelope,
            string queueName, IEnumerable<IncomeDto> listIncome)
        {
            if (listIncome != null)
            {
                var updatedResponse = JsonSerializer.Serialize(listIncome);
            ...
            else
            {
                await messagingService.UpdateAsync(new MessagingDto
                {
                    ...
                    QueueName = queueName,
                    MessageRequest = string.Empty,
                    MessageResponse = JsonSerializer.Serialize(listIncome),
                    IsProcessed = false,
                    ErrorMessage = "Failed to insert expense category into database.",
                    UpdatedAt = DateTime.Now,
                    IsActive = false
        ...
        private async Task ProcessedMessageGetByIdAsync(IMessagingAppService messagingService, IncomeMessage envelope,
            string queueName, IncomeDto expenseCategory)
        {
        ...
        private async Task ProcessedMessageUpdateAsync(IMessagingAppService messagingService, IncomeMessage envelope,
        ...
        private async Task ProcessedMessageDeleteAsync(IMessagingAppService messagingService, IncomeMessage envelope,
            string queueName, bool isDone)
        {
            if (isDone)
            {
