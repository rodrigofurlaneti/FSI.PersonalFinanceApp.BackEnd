...
{
    public class ExpenseConsumer : BackgroundService
    {
        private readonly IServiceScopeFactory _scopeFactory;
        private readonly IConfiguration _config;

        public ExpenseConsumer(IServiceScopeFactory scopeFactory, IConfiguration config)
        {
        ...
        {
            string queueName = "expense-queue";
            ...
            consumer.Received += async (sender, ea) =>
            {
                try
                {
                    ...
                    var envelope = JsonSerializer.Deserialize<ExpenseMessage>(
                        message,
                    ...
                    var service = scope.ServiceProvider.GetRequiredService<IExpenseAppService>();
                    ...
                    IEnumerable<ExpenseDto> listExpense = null;
                    ...
                    switch (envelope.Action.ToLowerInvariant())
                    {
                        case "create":
                            ...
                            break;
                        case "getall":
                            listExpense = await service.GetAllAsync();
                            break;
                        case "getbyid":
                    ...
                    await ProcessedMessageAsync(messagingService, envelope, queueName, createdId, isDone, listExpense, expenseCategory);
        ...
        private async Task ProcessedMessageAsync(IMessagingAppService messagingService, ExpenseMessage envelope, string queueName, long? createdId, bool isDone,
            IEnumerable<ExpenseDto> listExpense, ExpenseDto expenseCategory)
        {
            ...
            {
                await ProcessedMessageGetAllAsync(messagingService, envelope, queueName, listExpense);
            }
        ...
        private async Task ProcessedMessageCreateAsync(IMessagingAppService messagingService, ExpenseMessage envelope,
        ...
        private async Task ProcessedMessageGetAllAsync(IMessagingAppService messagingService, ExpenseMessage envelope,
            string queueName, IEnumerable<ExpenseDto> listExpense)
        {
            if (listExpense != null)
            {
                var updatedResponse = JsonSerializer.Serialize(listExpense);
            ...
            else
            {
                await messagingService.UpdateAsync(new MessagingDto
                {
                    ...
                    QueueName = queueName,
                    MessageRequest = string.Empty,
                    MessageResponse = JsonSerializer.Serialize(listExpense),
                    IsProcessed = false,
                    ErrorMessage = "Failed to insert expense category into database.",
                    UpdatedAt = DateTime.Now,
                    IsActive = false
        ...
        private async Task ProcessedMessageGetByIdAsync(IMessagingAppService messagingService, ExpenseMessage envelope,
            string queueName, ExpenseDto expenseCategory)
        {
        ...
        private async Task ProcessedMessageUpdateAsync(IMessagingAppService messagingService, ExpenseMessage envelope,
        ...
        private async Task ProcessedMessageDeleteAsync(IMessagingAppService messagingService, ExpenseMessage envelope,
            string queueName, bool isDone)
        {
            if (isDone)
            {
