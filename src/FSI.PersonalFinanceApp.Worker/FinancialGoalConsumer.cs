...
{
    public class FinancialGoalConsumer : BackgroundService
    {
        private readonly IServiceScopeFactory _scopeFactory;
        private readonly IConfiguration _config;

        public FinancialGoalConsumer(IServiceScopeFactory scopeFactory, IConfiguration config)
        {
        ...
        {
            string queueName = "financial-goal-queue";
            ...
            consumer.Received += async (sender, ea) =>
            {
                try
                {
                    ...
                    var envelope = JsonSerializer.Deserialize<FinancialGoalMessage>(
                        message,
                    ...
                    var service = scope.ServiceProvider.GetRequiredService<IFinancialGoalAppService>();
                    ...
                    IEnumerable<FinancialGoalDto> listFinancialGoal = null;

                    FinancialGoalDto expenseCategory = null;
                    ...
                    switch (envelope.Action.ToLowerInvariant())
                    {
                        case "create":
                            ...
                            break;
                        case "getall":
                            listFinancialGoal = await service.GetAllAsync();
                            break;
                        case "getbyid":
                    ...
                    await ProcessedMessageAsync(messagingService, envelope, queueName, createdId, isDone, listFinancialGoal, expenseCategory);
        ...
        private async Task ProcessedMessageAsync(IMessagingAppService messagingService, FinancialGoalMessage envelope, string queueName, long? createdId, bool isDone,
            IEnumerable<FinancialGoalDto> listFinancialGoal, FinancialGoalDto expenseCategory)
        {
            ...
            {
                await ProcessedMessageGetAllAsync(messagingService, envelope, queueName, listFinancialGoal);
            }
        ...
        private async Task ProcessedMessageCreateAsync(IMessagingAppService messagingService, FinancialGoalMessage envelope,
        ...
        private async Task ProcessedMessageGetAllAsync(IMessagingAppService messagingService, FinancialGoalMessage envelope,
            string queueName, IEnumerable<FinancialGoalDto> listFinancialGoal)
        {
            if (listFinancialGoal != null)
            {
                var updatedResponse = JsonSerializer.Serialize(listFinancialGoal);
            ...
            else
            {
                await messagingService.UpdateAsync(new MessagingDto
                {
                    ...
                    QueueName = queueName,
                    MessageRequest = string.Empty,
                    MessageResponse = JsonSerializer.Serialize(listFinancialGoal),
                    IsProcessed = false,
                    ErrorMessage = "Failed to insert expense category into database.",
                    UpdatedAt = DateTime.Now,
                    IsActive = false
        ...
        private async Task ProcessedMessageGetByIdAsync(IMessagingAppService messagingService, FinancialGoalMessage envelope,
            string queueName, FinancialGoalDto expenseCategory)
        {
        ...
        private async Task ProcessedMessageUpdateAsync(IMessagingAppService messagingService, FinancialGoalMessage envelope,
        ...
        private async Task ProcessedMessageDeleteAsync(IMessagingAppService messagingService, FinancialGoalMessage envelope,
            string queueName, bool isDone)
        {
            if (isDone)
            {
