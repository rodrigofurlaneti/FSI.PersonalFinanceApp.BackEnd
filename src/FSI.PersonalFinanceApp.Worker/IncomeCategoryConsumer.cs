...
{
    public class IncomeCategoryConsumer : BackgroundService
    {
        private readonly IServiceScopeFactory _scopeFactory;
        private readonly IConfiguration _config;

        public IncomeCategoryConsumer(IServiceScopeFactory scopeFactory, IConfiguration config)
        {
        ...
        {
            string queueName = "income-category-queue";
            ...
            consumer.Received += async (sender, ea) =>
            {
                try
                {
                    ...
                    var envelope = JsonSerializer.Deserialize<IncomeCategoryMessage>(
                        message,
                    ...
                    var service = scope.ServiceProvider.GetRequiredService<IIncomeCategoryAppService>();
                    ...
                    IEnumerable<IncomeCategoryDto> listIncomeCategory = null;

                    IncomeCategoryDto expenseCategory = null;
                    ...
                    switch (envelope.Action.ToLowerInvariant())
                    {
                        case "create":
                            ...
                            break;
                        case "getall":
                            listIncomeCategory = await service.GetAllAsync();
                            break;
                        case "getbyid":
                    ...
                    await ProcessedMessageAsync(messagingService, envelope, queueName, createdId, isDone, listIncomeCategory, expenseCategory);
        ...
        private async Task ProcessedMessageAsync(IMessagingAppService messagingService, IncomeCategoryMessage envelope, string queueName, long? createdId, bool isDone,
            IEnumerable<IncomeCategoryDto> listIncomeCategory, IncomeCategoryDto expenseCategory)
        {
            ...
            {
                await ProcessedMessageGetAllAsync(messagingService, envelope, queueName, listIncomeCategory);
            }
        ...
        private async Task ProcessedMessageCreateAsync(IMessagingAppService messagingService, IncomeCategoryMessage envelope,
        ...
        private async Task ProcessedMessageGetAllAsync(IMessagingAppService messagingService, IncomeCategoryMessage envelope,
            string queueName, IEnumerable<IncomeCategoryDto> listIncomeCategory)
        {
            if (listIncomeCategory != null)
            {
                var updatedResponse = JsonSerializer.Serialize(listIncomeCategory);
            ...
            else
            {
                await messagingService.UpdateAsync(new MessagingDto
                {
                    ...
                    QueueName = queueName,
                    MessageRequest = string.Empty,
                    MessageResponse = JsonSerializer.Serialize(listIncomeCategory),
                    IsProcessed = false,
                    ErrorMessage = "Failed to insert expense category into database.",
                    UpdatedAt = DateTime.Now,
                    IsActive = false
        ...
        private async Task ProcessedMessageGetByIdAsync(IMessagingAppService messagingService, IncomeCategoryMessage envelope,
            string queueName, IncomeCategoryDto expenseCategory)
        {
        ...
        private async Task ProcessedMessageUpdateAsync(IMessagingAppService messagingService, IncomeCategoryMessage envelope,
        ...
        private async Task ProcessedMessageDeleteAsync(IMessagingAppService messagingService, IncomeCategoryMessage envelope,
            string queueName, bool isDone)
        {
            if (isDone)
            {
